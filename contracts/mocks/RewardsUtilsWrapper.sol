...
contract RewardsUtilsWrapper {
    ...
    function updateRewardPeriod(uint256 totalStaked, uint256 rewardPerSecond, uint256 endDate) external {
        rewardsState.updateRewardPeriod(totalStaked, rewardPerSecond, endDate);
    }

    function earnedReward(uint256 totalStaked, address depositor, uint256 staked) external view returns (uint256) {
        return rewardsState.earnedReward(totalStaked, depositor, staked);
    }

    function updateDepositorReward(uint256 totalStaked, address depositor, uint256 staked)
    external returns (uint256) {
        return rewardsState.updateDepositorReward(totalStaked, depositor, staked);
    }

    function payDepositorReward(uint256 totalStaked, address depositor, uint256 staked) external returns (uint256) {
